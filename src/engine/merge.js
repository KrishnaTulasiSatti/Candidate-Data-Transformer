 ...
 * Handles overlapping date ranges by merging them before summing.
 * "Present" is treated as today's date.
 */
function calculateYearsExperience(experienceList) {
  ...
  // Parse each entry into [startMs, endMs]
  const ranges = [];
  ...
  // Sum total duration in years (rounded to 1 decimal)
...
function mergeProfiles(profilesWithSource, config) {
  ...
  // Compute years_experience from merged experience list
