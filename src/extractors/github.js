...
async function extractGithub(url) {
  try {
    ...
    if (data.name) profile.full_name = normalizeFullName(data.name);
    if (data.email) {
    ...
    if (data.location) profile.location = normalizeLocation(data.location);
    if (data.bio) profile.headline = data.bio;
    
    profile.links.github = data.html_url;
    
    // Fetch repos to get languages and repo list
    if (data.repos_url) {
      try {
